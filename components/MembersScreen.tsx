...
import { EditMemberModal, InviteModal, AddMemberModal } from './ManagementModals';
...
interface Props {
  ...
  onAddMember?: (data: { name: string; whatsapp: string; birthDate?: string; ministry_functions: string[] }) => Promise<void>;
...
export const MembersScreen: React.FC<Props> = ({
  ...
  const handleAddMemberClick = () => {
    if (!isAdmin) {
      addToast('Somente administradores podem adicionar membros manualmente.', 'warning');
      return;
    }
    ...
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto pb-28">
        ...
        <div className="flex flex-col gap-6 border-b border-zinc-200 dark:border-zinc-700 pb-6">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
               ...
               <div className="flex items-center gap-2">
                   <button
                       ...
                       className="hidden md:flex items-center gap-2 px-4 py-2 bg-ministral-500 hover:bg-ministral-600 text-white rounded-lg text-xs font-bold transition-all shadow-lg shadow-ministral-500/20 active:scale-95"
                   >
                       <UserPlus size={16}/> Adicionar Manual
                   </button>
           ...
           <div className="flex flex-col md:flex-row gap-3">
               <button
                   onClick={handleAddMemberClick}
                   className="md:hidden w-full flex items-center justify-center gap-2 px-4 py-3 bg-ministral-500 hover:bg-ministral-600 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-ministral-500/20 active:scale-95 mb-2"
               >
                   <UserPlus size={18}/> Adicionar Manual
               </button>
        ...
        {filteredMembers.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 auto-rows-fr items-stretch">
            {filteredMembers.map(member => {
                const isOnline = onlineUsers.includes(member.id);
                const isSelf = currentUser.id === member.id;
                const memberFunctions = member.ministry_functions || [];
                const isExpanded = expandedCards[member.id] !== false; // expanded by default

                return (
                <div key={member.id} className="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5 flex flex-col relative group shadow-sm hover:shadow-md transition-all hover:border-zinc-300 dark:hover:border-zinc-700 animate-slide-up h-full">
                    {/* Row 1: Avatar + Full Name + Expander */}
                    <div className="flex items-center gap-3">
                        <div className="relative shrink-0">
                            {member.avatar_url ? (
                                <img src={member.avatar_url} alt={member.name} className="w-12 h-12 rounded-full object-cover border-2 border-zinc-100 dark:border-zinc-700 shadow-sm" />
                            ) : (
                                <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-secondary to-secondaryHover flex items-center justify-center text-white text-lg font-bold border-2 border-zinc-100 dark:border-zinc-700 shadow-sm">
                                    {member.name.charAt(0).toUpperCase()}
                                </div>
                            )}
                            {isOnline && (
                                <div className="absolute bottom-0 right-0 w-3 h-3 bg-secondary rounded-full border-2 border-white dark:border-[#18181b] animate-pulse shadow-sm" title="Online Agora"></div>
                            )}
                        </div>
                        <h3 className="font-bold text-base text-zinc-800 dark:text-zinc-100 flex-1 min-w-0 break-words leading-tight min-h-[2.5rem] flex items-center" title={member.name}>
                            {member.name} {isSelf && <span className="text-secondary dark:text-white text-xs ml-1">(Você)</span>}
                        </h3>
                        <button
                            onClick={() => setExpandedCards(prev => ({ ...prev, [member.id]: !isExpanded }))}
                            aria-label={isExpanded ? `Recolher detalhes de ${member.name}` : `Expandir detalhes de ${member.name}`}
                            className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-all shrink-0"
                        >
                            <ChevronDown size={18} className={`transition-transform duration-200 ${isExpanded ? 'rotate-180' : ''}`} />
                        </button>
                    </div>

                    {/* Row 2: Role Badge + Admin Actions */}
                        ...
                        {!isSelf && (
                            <div className="flex items-center gap-1">
                                ...
                                <button 
                                    ...
                                    aria-label={member.isAdmin ? `Remover admin de ${member.name}` : `Tornar ${member.name} admin`}
                                >
                                    <Shield size={15} fill={member.isAdmin ? "currentColor" : "none"} />
                                </button>
                    ...
                    {/* Expanded Area: Functions + Contact */}
                    {isExpanded && (
                        <div className="mt-3 pt-3 border-t border-zinc-100 dark:border-zinc-800/50 space-y-3 animate-fade-in">
                            {/* Functions */}
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex items-center gap-3 text-zinc-500 dark:text-zinc-400 group/item hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors">
                                    <Mail size={15} className="text-zinc-300 dark:text-zinc-600 shrink-0"/>
                                ...
                                <div className="flex items-center justify-between text-zinc-500 dark:text-zinc-400 group/item hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors">
                                    <div className="flex items-center gap-3 truncate">
                                        <MessageCircle size={15} className="text-zinc-300 dark:text-zinc-600 shrink-0"/>
                                ...
                                {member.birthDate && (
                                    <div className="flex items-center gap-3 text-zinc-500 dark:text-zinc-400 group/item hover:text-zinc-800 dark:hover:text-zinc-200 transition-colors">
                                        <Gift size={15} className="text-zinc-300 dark:text-zinc-600 shrink-0"/>
        ...
        {/* Add Member Manual Modal */}
        {isAddMemberOpen && currentUser.ministryId && currentUser.organizationId && (
            <AddMemberModal
                ...
                onAdd={onAddMember || (async () => {
                    console.error('onAddMember handler not provided');
                })}
            />
        )}
        </div>
